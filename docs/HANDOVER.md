...
| Thing | State |
|---|---|
| `main` | Released app **v1.2.0** (tag `v1.2.0`, signed APK on the Releases page, installed via Obtainium). 1.2.0 (2026-09-20, MacBook): single-row home header, and the Stashwise card (`ui/promo/StashwiseCard.kt`) on home and in Settings. |
...
| branch `feat/v1.1.0` | App v1.1.0 in progress; unit tests pass at the last commit. **Done:** page-widget fallback collector (second WebView pass), early `NO_FEED` end for starved scans, VPN hint in the banner, scan-history labels, Room **migration 1→2** (`posts.imageUrl`, `posts.imagePath`) with exported schema, **relay-feed reader** (2026-09-19: `RelayFeedParser`, `RelayFeedFetcher`, `RelayFreshness`, `ScanCoordinator` asks the relay first, `CollectorKind.RELAY` labelled "Home collector", diagnostics line `relay=fresh|stale|unreachable|invalid`; not yet tried on the phone). Also done 2026-09-19: `ImageStore` (download once, orphan sweep), photo on the card (strip when closed, whole when opened), version **1.1.0 / code 4**, README privacy section. **NOT done:** **Map button** (`geo:` intent, OSM fallback) — deliberately left out of 1.1.0; spec update. |
...
2. **App relay reader — DONE 2026-09-19 on the MacBook, kept as the record of what was asked for** (`feat/v1.1.0`): `RelayFeedParser` (pure, hostile-input safe, version 1 only, image host must end with `.fbcdn.net`), `RelayFeedFetcher` (HTTPS GET, ≤ 2 MB, 10 s timeouts, cache-busting `?t=<epochMinutes>`), and in `ScanCoordinator`: fetch the relay first; if fresh (`generatedAt` ≤ 30 min — 20 was too tight for the 15-minute heartbeat plus GitHub's cache — and `lastFullScanAt` ≤ 60 min old) record its posts with `CollectorKind.RELAY` and skip the WebView; otherwise run the existing pipeline and still merge the relay's posts. Labels: "Home collector". Diagnostics lines `relay=fresh|stale|unreachable|invalid`.
